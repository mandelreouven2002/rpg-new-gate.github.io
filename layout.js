/**
 * קובץ זה חוסך שכפול קוד. ממוקם בתיקיית השורש (Root).
 */
...
function injectHeader() {
    // שימו לב לנתיבים האבסולוטיים (מתחילים ב-/) כדי שיעבדו מכל תיקיית משנה
    const headerHTML = `
        ...
        <div class="container mx-auto px-4 md:px-6 py-3 flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
            ...
            <nav class="flex w-full md:w-auto flex-wrap justify-center md:justify-end items-center gap-4 md:gap-6 font-medium text-sm md:text-base border-t border-indigo-800 pt-3 md:border-0 md:pt-0">
                <div class="flex items-center gap-4 md:gap-6" id="global-nav-links">
                    <a href="/" class="nav-item hover:text-yellow-400 transition-colors" data-path="index">בית</a>
                    <a href="/guide.html" class="nav-item hover:text-yellow-400 transition-colors" data-path="guide">המדריך</a>
                    <a href="/go/search.html" class="nav-item hover:text-yellow-400 transition-colors" data-path="go">חיפוש</a>
                    <a href="/articles/articles.html" class="nav-item hover:text-yellow-400 transition-colors" data-path="articles">מאמרים</a>
                    <a href="/codex/codex.html" class="nav-item hover:text-yellow-400 transition-colors" data-path="codex">קודקס</a>
                    <a href="/msg/msg.html" class="nav-item hover:text-yellow-400 transition-colors" data-path="msg">מבזקון</a>
                </div>
...
function injectFooter() {
    ...
    <footer class="text-center py-8 mt-auto bg-stone-200 border-t border-stone-300 w-full">
        ...
        <p class="text-sm text-stone-500 font-bold">
            אנחנו בארגון גמ"ה - גלגול מיזמים התנדבותיים, גאים לבנות את הקהילה.
        </p>
    </footer>
    `;
...
function highlightActiveLink() {
    ...
    navItems.forEach(item => {
        ...
        // התאמה לתיקיות (למשל אם אנחנו ב- /codex/ משהו)
        if (pathData !== 'index' && currentPath.includes(`/${pathData}`)) {
            item.classList.add('text-yellow-400', 'font-bold');
            activeMatched = true;
        }
    });

    // אם אנחנו בשורש (/) נסמן את הבית
    if (!activeMatched && (currentPath === '/' || currentPath.endsWith('index.html'))) {
